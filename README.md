...
地球本体启用 Cesium 天空大气和地表大气参数，首屏会保留贴合球体边缘的蓝色空气辉光。默认展示模式使用无硬昼夜分界的浏览光照，避免亚洲首屏被太阳终止线切开；需要真实太阳时间时仍可通过 `sunlight=1`、工作台开关或 `setSunlight(true)` 启用。这些都是底座视觉处理，不会写入业务 GeoJSON，也不会影响天气图层透明度。
